...
// === HIỂN THỊ DANH SÁCH THÀNH VIÊN TRÊN CARD ===

String expectedMember = "Lê Thị Thanh Ngân"

// Locator avatar của thành viên trên Card
TestObject memberAvatars = new TestObject("memberAvatars")
memberAvatars.addProperty(
	"xpath",
	ConditionType.EQUALS,
	"//button[@data-testid='card-back-member-avatar']"
)

// Chờ UI Trello load avatar
WebUI.delay(2)

// Lấy danh sách avatar
List<WebElement> avatarList = WebUI.findWebElements(memberAvatars, 10)

// Debug xem tìm được bao nhiêu avatar
println("Tổng số avatar hiển thị trên Card: " + avatarList.size())

// ❗ Kiểm tra Card có ít nhất 1 member
WebUI.verifyGreaterThan(avatarList.size(), 0)

// --- Kiểm tra avatar đúng thành viên đã thêm ---

TestObject avatarWithName = new TestObject("avatarWithName")
avatarWithName.addProperty(
	"xpath",
	ConditionType.EQUALS,
	"//button[@data-testid='card-back-member-avatar' and contains(@title,'" + expectedMember + "')]"
)

// Kiểm tra avatar có tồn tại
WebUI.verifyElementPresent(avatarWithName, 10)

// In giá trị PASS
println("✔ PASS — Avatar thành viên '" + expectedMember + "' hiển thị đúng trên Card!")
