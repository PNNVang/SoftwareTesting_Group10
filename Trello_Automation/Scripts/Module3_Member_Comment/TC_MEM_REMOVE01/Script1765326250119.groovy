...
// === Re-open member avatar ===
TestObject cardMemberAvatar = new TestObject('cardMemberAvatar')
cardMemberAvatar.addProperty(
	"xpath",
	ConditionType.EQUALS,
	"//button[@data-testid='card-back-member-avatar' and contains(@title, 'Lê Thị Thanh Ngân')]"
)

WebUI.waitForElementVisible(cardMemberAvatar, 10)
WebUI.waitForElementClickable(cardMemberAvatar, 10)
WebUI.click(cardMemberAvatar)
WebUI.delay(1)

// === REMOVE MEMBER ===
TestObject removeMember = new TestObject("removeMember")
removeMember.addProperty(
	"xpath",
	ConditionType.EQUALS,
	"//li[@data-testid='remove-from-card']//button"
)

WebUI.waitForElementVisible(removeMember, 10)
WebUI.waitForElementClickable(removeMember, 10)
WebUI.click(removeMember)
WebUI.delay(1)

// === VERIFY MEMBER REMOVED ===
TestObject removedCheck = new TestObject("removedCheck")
removedCheck.addProperty(
	"xpath",
	ConditionType.EQUALS,
	"//button[@data-testid='card-back-member-avatar' and contains(@title, 'Lê Thị Thanh Ngân')]"
)

WebUI.verifyElementNotPresent(removedCheck, 5)
